...
class EEG_insight(object):
    ...
    def update_screen(self):
        ...
        while 1:
            ...
            if "csv" not in args['omit']:
                with open(filename, 'a') as f:
                    f.write(','.join(line) + "\n")

            if "osc" not in args['omit']:
                client.send_message("/AF3", eeg_data[EEG_name["AF3"]])
                client.send_message("/T7", eeg_data[EEG_name["T7"]])
                client.send_message("/Pz", eeg_data[EEG_name["Pz"]])
                client.send_message("/T8", eeg_data[EEG_name["T8"]])
                client.send_message("/AF4", eeg_data[EEG_name["AF4"]])
